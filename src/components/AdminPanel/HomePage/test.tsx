...
const HomeDataDisplay = () => {
  ...
  const handleImageDelete = async (key: string) => {
    if (!homeData || !editedData) return;

    const imageUrl = editedData.images[key];
    const publicId = imageUrl?.split("/").pop()?.split(".")[0]; // Assuming the public ID is the part before the file extension

    if (publicId) {
      // Delete the image from Cloudinary
      await axios.delete(
        `https://api.cloudinary.com/v1_1/duox5d29k/image/destroy`,
        {
          data: { public_id: publicId, api_key: "your_api_key" }, // Replace with your Cloudinary API key
        }
      );

      // Remove the image from the edited data
      const updatedImages = { ...editedData.images };
      delete updatedImages[key];

      // Update Firestore with the updated images
      const homeRef = doc(db, "homeData", editedData.id);
      await updateDoc(homeRef, { ...editedData, images: updatedImages });
      setHomeData({ ...editedData, images: updatedImages });
